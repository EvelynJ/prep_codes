...
pro MUSE_psf_NGC1396


root='/raid/ejohnston/NGC1396/BUDDI/'
  side=25.0
...
;  fits_read,'/raid/ejohnston/NGC1396/products_std/DATACUBE_FINAL_ZAPPED.fits',input,h
  fits_read,'/raid/ejohnston/NGC1396/BUDDI/DATACUBE_FINAL_ZAPPED.fits',input,h
  ...
  x_in=[271, 252, 149, 100,  50,  89]   ;using science datacube
  y_in=[ 48,  82,  70,  91, 156, 188]
  x_in=[271,  149, 100,  50,  89, 337, 484, 546, 495]   ;using science datacube
  y_in=[ 48,   70,  91, 156, 188,  54,  61,  86, 138]
  x_in=[271,  100,  50,  89,  484, 546, 548]   ;using science datacube
  y_in=[ 48,   91, 156, 188,   61,  86, 297]
...
;;;;median stack psf images
;    input_small=fltarr(2*side+1,2*side+1,n_elements(x_in))
;    for n=0,n_elements(x_in)-1,1 do begin
;      input_small[*,*,n]=median(input[x_in[n]-side:x_in[n]+side,y_in[n]-side:y_in[n]+side,m-50:m+50],dimension=3)
;    endfor
;    psf_new=median(input_small,dimension=3)
;    psf_new=psf_new/total(psf_new)
...
;  psf_cube_final=psf_cube[1:*,1:*,*]
  ...
  psf_cube_final=fltarr(2*side+1,2*side+1,sxpar(h,'NAXIS3'))
  psf_cube_final[0:-2,0:-2,*]=psf_cube2[1:*,1:*,*]
